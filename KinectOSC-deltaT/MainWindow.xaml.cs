...
namespace KinectOSC
{
    ...
    {
        string RemoteAddress = "192.168.1.101";
        ...
        private static UdpWriter oscWriter2;
...
//            Console.WriteLine("Size of master list: " + masterSkeletonList.Count);
        ...
        // 0 is off, 1 is on
    /*    private void AnimataOSCLayerToggle(string layer, int isOn)
        {
            var oscMessageArgs = new List<OscElement>();
            new OscElement("/layervis" + layer + isOn);
                oscWriter.Send(new OscBundle(DateTime.Now, oscMessageArgs.ToArray()));
        }
        */
    ...
    //            string jointName = "s" + counter + joint.JointType.ToString();
        ...
        private void ChangePortClicked(object sender, RoutedEventArgs e)
        {
            oscArgs[1] = OscPort.Text;
            oscWriter = new UdpWriter(oscArgs[0], Convert.ToInt32(oscArgs[1]));
            oscWriter2 = new UdpWriter(RemoteAddress, Convert.ToInt32(oscArgs[1]));
        }
